...
bd work ww-xyz --complexity=complex   # Override
bd work ww-xyz --model=opus           # Override
bd work ww-xyz --minimal              # Skip overhead
```
...
git log --oneline -10
bd ready | head -5
pnpm dev --filter=<package> &  # or: ./init.sh
```
...
| Repo | Prefix | Constraints |
|------|--------|-------------|
| Cloudflare | `ww-` | SDK, CLI, integrations, workflows, workers |
| workway-platform | `wwp-` | API, database, web UI, auth, teams |

**Cross-repo work**: Create linked issues, use `bd dep add` for dependencies.
